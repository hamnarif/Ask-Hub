...
const Chats: React.FC<ChatsProps> = ({ messages }) => {
    ...
    return (
        <div className="h-full overflow-y-auto pr-4 space-y-4 scrollbar-custom">
            {messages.length === 0 ? (
                        ...
                        <div key={index} className="space-y-4">
                            ...
                            {(message.user || message.file) && (
                                <div className="flex justify-end">
                                    <div className="max-w-[80%] text-right bg-stone-900 text-white rounded-xl p-4 space-y-2">
                                        {/* PDF File (if present) */}
                                        {message.file && (
                                            <div className="bg-red-700 text-white rounded-full px-3 py-1 w-fit flex items-center mx-auto">
                                                ...
                                                <span className="break-all">{message.file}</span>
                                            </div>
                                        )}
                                        {/* User Text */}
                                        {message.user && (
                            ...
                            {message.bot && (
                                <div className="flex justify-start">
                                    <div className="max-w-[80%] text-left text-stone-100 rounded-xl p-4">
                                        <div className="whitespace-pre-wrap break-words">{message.bot}</div>
                                    </div>
                                </div>
                            )}
                        </div>
