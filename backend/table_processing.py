...
def process_pdf_to_summary(pdf_path):
    try:
        ...
        # # Step 3.1: Print the JSON output and ask for user approval
        # print("\nFormatted JSON Output from Step 3:")
        # print(formatted_json)

        # # Deserialize the JSON for editing
        # json_data = orjson.loads(formatted_json)

        # # Function to display JSON with indexing
        # def display_json_with_index(data, prefix=""):
        ...
        # # Display the JSON in an editable format
        # print("\nEditable JSON Output:")
        # display_json_with_index(json_data)

        # # Prompt user for approval or editing
        # while True:
        #     user_response = input("\nDo you approve this output? (yes/edit/no): ").strip().lower()
        #     if user_response == 'yes':
        #         break  # Exit the loop if the user approves
        #     elif user_response == 'edit':
        #         # Allow the user to specify what to edit
        #         edit_path = input("\nEnter the path to the value to edit (e.g., [0][1] for nested lists or key for dictionaries): ").strip()
        #         try:
        #             # Navigate to the specified value
        #             path_parts = edit_path.replace("[", ".").replace("]", "").split(".")
        #             current = json_data
        ...
        #             old_value = current[int(last_part)] if last_part.isdigit() else current[last_part]
        ...
        #             display_json_with_index(json_data)
        #         except Exception as e:
        #             print(f"Invalid path or operation. Please try again. Error: {e}")
        #     elif user_response == 'no':
        #         print("Process terminated by the user.")
        #         return  # Exit the function if the user doesn't approve
        #     else:
        #         print("Invalid input. Please respond with 'yes', 'edit', or 'no'.")

        # # Re-serialize the updated JSON back into formatted JSON
        # formatted_json = orjson.dumps(json_data, option=orjson.OPT_INDENT_2).decode("utf-8")
